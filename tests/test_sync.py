...
from aidlc.schemas.profile import StepName
...
def test_agents_md_lists_every_ci_step_for_a_uv_target(tmp_path: Path) -> None:
    """AGENTS.md says "they are what CI runs", so every step CI runs has to
    be there. `format` and `audit` are the ones a hard-coded list left out."""
    repo = tmp_path / "repo"
    shutil.copytree(FIXTURES / "python-uv", repo)
    init(repo)
    agents = read(repo, "AGENTS.md")

    assert "- format: `" in agents
    assert "- audit: `uv run --with pip-audit pip-audit`" in agents

    names = {step.value for step in StepName}
    listed = [
        line[2:].split(":", 1)[0]
        for line in agents.splitlines()
        if line.startswith("- ") and line[2:].split(":", 1)[0] in names
    ]
    in_ci_order = [step.value for step in StepName if step.value in listed]
    assert listed == in_ci_order, "steps must follow CI order"
...
def test_init_writes_mcp_config_and_the_backend_agent_for_a_fastapi_service(
    tmp_path: Path,
) -> None:
    """The `mcp:` block reaches `.mcp.json` under every server's name, and the
    FastAPI pack brings the `backend` agent that grants those servers. The two
    are only useful together, so they are asserted together."""
    repo = tmp_path / "repo"
    ...
    assert header["name"] == "backend"
    assert "mcp__context7" in header["tools"]
    assert "mcp__aws-docs" in header["tools"]
    assert "Generated by aidlc from pack rules-fastapi@" in backend
    assert (repo / ".claude/agents/infra.md").exists(), "the SAM template brings rules-aws"
    assert not (repo / ".claude/agents/frontend.md").exists(), "no React target here"
...
def test_init_writes_the_pm_agent(repo: Path) -> None:
    """`core` is selected everywhere, so every repository gets `pm`; without
    it the orchestrate skill's step 0 names an agent that does not exist."""
    init(repo)
    text = read(repo, ".claude/agents/pm.md")

    header = yaml.safe_load(text.split("---\n")[1])
    assert header["name"] == "pm"
    assert "Bash" in header["tools"]
    assert "Generated by aidlc from pack core@" in text
